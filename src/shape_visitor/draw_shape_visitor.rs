...
use crate::utils::{ parse_error::ParseError, color::Color, point::Point };
    ...
    pub fn write_image(&self, file_name: &String) -> Result<(), Box<dyn Error>> {
        return match self.image.save(file_name) {
            ...
            Err(err) => {
                ...
                return Err(Box::new(ParseError::new(new_error_string)));
            }
        };
    }
}
...
impl ShapeVisitor for DrawShapeVisitor {
    ...
    fn visit_square(&mut self, square: &Square) {
        ...
        self.visit_rectangle(&rectangle);
    }
    ...
    fn visit_polygon(&mut self, polygon: &Polygon) {
        ...
        flood_fill(&mut self.image, center.x, center.y, outline_color, fill_color);
    }
    ...
    fn visit_circle(&mut self, circle: &Circle) {
        ...
        //println!("{}", circle);
        flood_fill(&mut self.image, xc, yc, outline_color, fill_color);
    }
}
...
fn flood_fill(image: &mut RgbaImage, x_start: i32, y_start: i32,
              outline_color: Rgba<u8>, fill_color: Rgba<u8>) {

    if !is_pixel_inside_image(image, x_start, y_start) {
        eprintln!("Pixel not in image!");
        return;
    }
